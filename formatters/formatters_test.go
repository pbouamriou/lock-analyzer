...
func TestMarkdownFormatter(t *testing.T) {
	formatter, err := NewMarkdownFormatter("fr")
	if err != nil {
...
func TestMarkdownFormatterEnglish(t *testing.T) {
	formatter, err := NewMarkdownFormatter("en")
	if err != nil {
		t.Fatalf("Error creating markdown formatter: %v", err)
	}
...
func TestTextFormatter(t *testing.T) {
	formatter, err := NewTextFormatter("fr")
	if err != nil {
...
// TestTextFormatterEnglish tests the text formatter in English
func TestTextFormatterEnglish(t *testing.T) {
	formatter, err := NewTextFormatter("en")
	if err != nil {
		t.Fatalf("Error creating text formatter: %v", err)
	}
	...
	if !strings.Contains(content, "POSTGRESQL LOCK ANALYSIS REPORT") {
		t.Error("Text report must contain main title")
	}
	if !strings.Contains(content, "EXECUTIVE SUMMARY") {
		t.Error("Text report must contain summary section")
	}
	if !strings.Contains(content, "ACTIVE LOCKS") {
		t.Error("Text report must contain active locks section")
	}
	if !strings.Contains(content, "IMPROVEMENT SUGGESTIONS") {
		t.Error("Text report must contain suggestions section")
	}
...
func TestFormatterInterface(t *testing.T) {
	formatterText, err := NewTextFormatter("en")
	if err != nil {
		t.Fatalf("Error creating text formatter: %v", err)
	}
	formatterMarkdown, err := NewMarkdownFormatter("en")
	if err != nil {
		t.Fatalf("Error creating markdown formatter: %v", err)
	}
